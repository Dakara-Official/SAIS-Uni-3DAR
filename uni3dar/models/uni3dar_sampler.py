...
# 这个函数用于根据读取到的properties，创建条件特征，与molecule_energy_data_utils.py中的类似，但源代码的是将同一个条件复制batch_size份，这里是将读取到的batch_size个条件直接传递
def get_molecule_energy_cond(
    args, cond_data, dictionary, xyz_null_id, bsz=1
):
    ...
    if args.data_type == "molecule_energy":
        # 直接使用传入的多个能量值
        energy_arr = np.array(cond_data["properties"], dtype=np.float32).reshape(-1, 1)
...
def results_from_predictions(
    data_type, minimum_level, finished_results, **kwargs
):
    atom_results = []
    for result in finished_results:
        ...
        if data_type == "molecule_energy": # 原为if data_type == "molecule":
        ...
        #         # print(e)
        #         continue
    # 源代码中筛选生成分子的功能，为了遵循参赛规则，关闭该功能
    # atom_results = sorted(atom_results, key=lambda x: x[1], reverse=False)
    # num_selected = int(len(atom_results) * rank_ratio)

    # 源代码中是读取一个条件生成batch_size个分子，筛选后输出分子
    # 我这里通过修改条件特征的读取转换方式(函数get_molecule_energy_cond)，改成读取batch_size个条件，分别生成一个分子
    num_selected = int(len(atom_results)) # 生成分子全部输出
...
class Uni3DARSampler(Uni3DAR):
    ...
    def get_atom_predict_funcs(
        ...
        def atom_predict_func(y, batch_indices, tree_i):
            ...
            # ) 去除不需要的部分
            atom_func = atom_sampling_func # 保留需要的部分
    ...
    # 仿真上面被注释的crystal_cond_prefilling_with_data函数，构建molecule_energy_cond函数
    def molecule_energy_cond(
        self,
        data,
        batch_size,
        device,
