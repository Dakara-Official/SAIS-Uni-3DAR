...
# from .diffusion_prediction_head import DiffusionPredictionHead 不需要
...
class Uni3DAR(BaseUnicoreModel):
    ...
    def __init__(self, args, dictionary):
        ...
        total_levels = self.args.merge_level + (
            3 if self.args.data_type == "molecule_energy" else 2
        )
        ...
        # )  源代码开启条件生成的规则，改成如下规则，代码训练即生成脚本均设置data_type=molecule_energy
        self.is_conditional = self.args.data_type == "molecule_energy"
        ...
        AtomHead = AutoRegressivePredictionHead
        ...
        # self.init_crystal_cond_embeding() # 不需要
        self.init_molecule_energy_cond_embeding() # 初始化分子条件生成嵌入
    ...
    # 仿照init_crystal_cond_embeding函数创建init_molecule_energy_cond_embeding函数
    def init_molecule_energy_cond_embeding(self):
        if self.args.data_type == "molecule_energy":
            self.energy_token = self.dictionary["[ENERGY]"]
            self.energy_embed = nn.Sequential(
                Linear(1, self.args.emb_dim, init="bert"),
                RMSNorm(self.args.emb_dim),
                nn.GELU(),
                Linear(self.args.emb_dim, self.args.emb_dim, bias=False, init="final"),
            )
            self.energy_gate = Linear(
                self.args.emb_dim, self.args.emb_dim, bias=False, init="bert"
            )
    ...
    # 仿照上方被注释的forward_crystal_cond函数，创建forward_cond函数
    def forward_cond(self, bsz, dec_y, decoder_input, energy):
        c = 0
        if self.args.data_type == "molecule_energy":
            # 1. 定位能量标记位置
            energy_mask = (
                (decoder_input.level == self.args.merge_level + 1) & 
                (decoder_input.type == self.energy_token)
            )

            # 2. 确保批次一致性
            assert energy.size(0) == bsz, f"Batch size mismatch: energy {energy.size(0)} vs {bsz}"

            # 3. 能量嵌入处理
            energy_emb = self.energy_embed(energy.view(bsz,1).to(self.dtype)).view(-1, self.args.emb_dim)
            
            # 4. 应用dropout
            energy_emb = F.dropout(energy_emb, self.args.emb_dropout, self.training)
            
            # 5. 注入隐藏状态
            dec_y[energy_mask] += energy_emb
            
            # 6. 提取当前隐藏状态值
            cond = dec_y[energy_mask]
            
            # 7. 聚合到条件向量c 
            n_cond = 1  # 能量条件只有1个token
            c += cond.view(-1, n_cond, self.args.emb_dim).sum(dim=1)
        return dec_y, c
    ...
    def forward_model(
        ...
        **kwargs,
        ...
        if "energy" in kwargs and kwargs["energy"] is not None:
            ...
            # )
            dec_y, c = self.forward_cond(
                bsz, dec_y, decoder_input, kwargs["energy"]
            )
    ...
    def forward(self, batched_data):
        try:
            ...
            dec_y, _ = self.forward_model(
                ...
                decoder_input_len=batched_data["decoder_input_len"],
                decoder_input_max_len=int(batched_data["decoder_input_max_len"]),
                energy=batched_data["energy"] if self.args.data_type == "molecule_energy" else None,
            )
